    ...
    <div class="min-h-screen flex">
        <aside class="w-72 bg-slate-950 text-white hidden md:flex md:flex-col">
            <div class="p-6 border-b border-slate-800">
                ...
                <div class="text-sm text-slate-400 mt-1">Formularios y permisos</div>
            </div>
            ...
            <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
                ...
                <a href="{{ route('permisos.solicitud.create') }}" target="_blank" class="block px-4 py-3 rounded-xl transition text-slate-300 hover:bg-slate-800">Solicitud pública</a>
            </nav>

            <div class="p-4 border-t border-slate-800">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full px-4 py-3 rounded-xl bg-red-600 hover:bg-red-700 text-white">Cerrar sesión</button>
                </form>
            </div>
        </aside>
        ...
        <main class="flex-1 min-w-0">
            <header class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-slate-500">@yield('page_description', 'Administración RH')</p>
                </div>
            ...
            <section class="p-6">
                @if(session('success'))
                    <div class="mb-5 rounded-xl bg-green-100 border border-green-300 text-green-800 px-4 py-3">{{ session('success') }}</div>
                @endif
                @if(session('error'))
